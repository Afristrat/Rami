...
0. **✅ IMPORT DE FICHIERS → PPTX (LIVRÉ + browser-verified, commit `c1cb70c`)** : upload MD/PDF(texte)/Word/Excel sur `/presentations/new` (onglet « Depuis un fichier ») → extraction (`file-extract.ts`) → conversion LLM prompt L99 → deck éditable + PPTX. Vérifié MD + Excel (chiffres source préservés). ⚠️ Excel via **exceljs** (pas `xlsx`/SheetJS = vuln HIGH). PDF aplati/scanné = pas pris en charge (pas d'OCR). **PROCHAIN** : tester PDF/Word réels d'Amine + éventuel OCR pour PDF scannés.
