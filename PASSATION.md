...
- **!! SERVEUR DOWN (2026-06-12 ~16h20 UTC, EN COURS à la rédaction)** : 530 Cloudflare sur TOUS les domaines tunnelés (rami, proxy, db-rami, crawl4ai) **ET** SSH LAN injoignable (`.8` timeout ; scan /24 : aucun hôte n'accepte la clé serveurai). ≠ incident WAN session #6 (le serveur restait joignable en LAN) → **serveur éteint/planté, intervention physique Amine requise**. Conséquences : prod RAMI down ; migration `…014_content_sessions` **PAS appliquée** ; commit `fc65164` (persistance workflow) **volontairement NON poussé** (l'auto-deploy doit suivre la migration, pas la précéder — même si le code dégrade proprement sans la table).
- **→ Au retour du serveur, dans l'ordre** : (1) vérifier `hostname`/IP (DHCP !) et re-noter l'IP dans le coffre si changée ; (2) appliquer la migration `20260315000014_content_sessions.sql` sur db-rami (méthode base64 habituelle) + test RLS cross-tenant ; (3) `git push origin main` (déclenche le deploy de `fc65164`) ; (4) browser-verify prod de la reprise de brouillon (parcours partiel → refresh → bannière « Reprendre » → état restauré ; « Recommencer » → abandoned) + cleanup `content_sessions` du tenant test.
