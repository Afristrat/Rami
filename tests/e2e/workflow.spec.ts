...
test.describe("Workflow création de contenu", () => {
  ...
  test("Étape 1 : affiche le formulaire Brief complet", async ({ onboardedPage: page }) => {
  ...
  test("Étape 1 : barre de progression à 0%", async ({ onboardedPage: page }) => {
    await expect(page.getByText("0% complété")).toBeVisible()
  })

  test("Étape 1 : bouton Continuer bloqué si formulaire vide", async ({ onboardedPage: page }) => {
    await page.getByRole("button", { name: /continuer/i }).click()
    // Doit rester sur l'étape 1
    await expect(page.getByText("Brief & contexte")).toBeVisible()
  })

  test("Étape 1 : validation — titre trop court", async ({ onboardedPage: page }) => {
  ...
  test("Étape 1 : sélection objectif cognitif", async ({ onboardedPage: page }) => {
    await page.getByRole("button", { name: /confiance/i }).click()
    await expect(page.getByRole("button", { name: /confiance/i })).toHaveClass(/ring-/)
  })

  test("Étape 1 : XSS dans le brief — sanitisé", async ({ onboardedPage: page }) => {
  ...
  test("Étape 1 : passage à l'étape 2 avec données valides", async ({ onboardedPage: page }) => {
  ...
  test("Étape 2 : affiche les 8 plateformes", async ({ onboardedPage: page }) => {
  ...
  test("Étape 2 : affiche les 5 formats de contenu", async ({ onboardedPage: page }) => {
    await page.fill("#titre", "Test contenu")
    await page.fill("#description", "Description de test pour valider le workflow complet")
  ...
  test("Étape 2 : sélection multiple de plateformes", async ({ onboardedPage: page }) => {
    await page.fill("#titre", "Test contenu")
    await page.fill("#description", "Description de test pour valider le workflow complet")
  ...
  test("Étape 2 : bouton Retour revient à l'étape 1", async ({ onboardedPage: page }) => {
    await page.fill("#titre", "Test contenu")
    await page.fill("#description", "Description de test pour valider le workflow complet")
  ...
  test("Stepper affiche les étapes correctement", async ({ onboardedPage: page }) => {
  ...
  test("Page create accessible depuis le dashboard", async ({ onboardedPage: page }) => {
    await page.goto("/dashboard")
    await page.getByText("Créer du contenu").click()
    await expect(page).toHaveURL(/\/dashboard\/create/)
  })

  test("Page create : titre de la page", async ({ onboardedPage: page }) => {
    await expect(page).toHaveTitle(/Créer du contenu/)
  })

  // ── Étape 7 — Planification ───────────────────────────────────────────────

  test("Workflow complet : accès page create sans erreur 500", async ({ onboardedPage: page }) => {
